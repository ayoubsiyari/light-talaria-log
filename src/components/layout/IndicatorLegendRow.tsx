...
interface IndicatorLegendRowProps {
  ...
  onSettings: () => void;
  onRemove: () => void;
  /** Optional slot after the action buttons (e.g. settings popover anchor). */
  settingsSlot?: ReactNode;
}

/** Shared TV-style indicator legend chip (Volume + overlays). */
export function IndicatorLegendRow({
  ...
  return (
    <div
      className={[
        'group flex items-center gap-0.5 pl-0.5 pr-0.5 rounded-md text-xs min-h-7',
    ...
    >
      ...
      <span className="font-medium text-foreground truncate max-w-[9.5rem] px-1">
        {label}
      </span>
      ...
      >
        <IconButton
          ...
          ariaLabel={`${visible ? 'Hide' : 'Show'} ${label}`}
