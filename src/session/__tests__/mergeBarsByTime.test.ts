/**
 * Warm-cache bar merge. Run:
 * node --experimental-strip-types --import ./scripts/register-alias.mjs --test src/session/__tests__/mergeBarsByTime.test.ts
 */
...
describe('mergeBarsByTime', () => {
  ...
  it('does not drop newer tip when incoming window ends earlier', () => {
    ...
    assert.equal(out[out.length - 1]!.time, 151);
    assert.equal(out[0]!.time, 80);
  });
